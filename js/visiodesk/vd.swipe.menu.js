...
window.SwipeMenu = function (e, options) {
    ...
    function displayLeftMenu() {
        ...
        state = leftState;
        recentMarginLeft = -swipeMenuWidth * 0.4;
    }
    ...
    function displayRightMenu() {
        ...
        state = rightState;
        recentMarginLeft = -swipeMenuWidth -swipeMenuWidth * 0.6;
    }

    function defaultLeftAction() {
        const list = e.getElementsByClassName("item-left default-action");
    ...
    function defaultRightAction() {
        const list = e.getElementsByClassName("item-right default-action");
        if (list.length && onSelectCallback) {
            reset(() => {
                onSelectCallback(list[0]);
            });
    ...
    mc.on("pan", (evt) => {
        ...
        if (-70 <= totalSwipePercent && totalSwipePercent <= 70) {
            eSwipe.classList.remove("active-left", "active-right", "do");
        }

        if (totalSwipePercent < -70) {
            eSwipe.classList.remove("active-left", "active-right", "do");
    ...
    mc.on("panend", (evt) => {
        ...
        if (direction === Hammer.DIRECTION_RIGHT && -65 < totalSwipePercent && totalSwipePercent < 0) {
            return reset();
        }
        if (direction === Hammer.DIRECTION_LEFT && 0 < totalSwipePercent && totalSwipePercent < 65) {
            return reset();
        }

        if (-70 <= totalSwipePercent && totalSwipePercent < 0) {
            state.right();
        } else if (totalSwipePercent < -70) {
            defaultRightAction();
        }

        if (5 <= totalSwipePercent && totalSwipePercent < 70) {
            state.left();
        } else if (totalSwipePercent > 70) {
            defaultLeftAction();
        }
